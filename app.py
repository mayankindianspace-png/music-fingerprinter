...
def generate_hashes(constellation):
    ...
    for i, (f1, t1) in enumerate(constellation):
        for j in range(i + 1, min(i + FAN_VALUE + 1, n)):
            ...
            dt = t2 - t1
            if TIME_DELTA_MIN <= dt <= TIME_DELTA_MAX:
...
# Named function instead of lambda -- fixes pickle error with st.cache_resource
def make_int_dict():
...
class FingerprintDB:
    ...
    def match(self, query_audio, sr=SR):
        ...
        # Use named function not lambda to keep it picklable
        offset_hist = defaultdict(make_int_dict)
